...
- offline の隔離条件は2026-09-08のユーザー指示「そっちで決めて再開していい」に基づき実施側で決定する。人工認証・人工公開入力・専用 CODEX_HOME と user/temp ディレクトリ・継承変数の allowlist を使用し、生成先・認証更新先・metadata 先を127.0.0.1に固定する。HTTP proxy は外部宛てを転送せず拒否する localhost receiver とし、analytics/feedback/拡張を無効化する。これは信頼する固定版 CLI の通信先設定による隔離であり、OS レベルの全通信遮断や悪意あるプロセスに対する防壁を保証しない。受信経路・実 payload・再送数を実測する。OS Firewall は変更しない。
...
- status: done
- evidence: 2026-09-08 `verify-offline` exit 0。正常 HTTP=1、401 復旧/打切り HTTP=3・refresh=1、SSE idle timeout/503/成立後切断 HTTP=1。全ケース tools=[]、完全な公開 ProviderRequest、trace と receiver の request 一致。詳細は `blocked/T-001.md` の再開結果。
- done-when: 承認済み一時 root 内に標準ライブラリの probe と公開設定があり、通信先固定・proxy の外部宛て拒否・実認証なし、最終 tools=[]、非公開入力混入なし、正常/401/timeout/5xx/切断の最小ケースの source と受信数と trace の対応を evidence/offline に記録する。verify-offline が保存済みの証拠を開いて検証し exit 0。再現できない必須分岐は合格でなく blocked とする。
